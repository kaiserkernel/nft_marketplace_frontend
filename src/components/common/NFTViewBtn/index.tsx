...
export const NFTViewBtn = ({
  ...
  const handleClickBuyNft = async () => {
    ...
    await handleBuyNft(price, tokenId, currency);
  };
    ...
    >
      {nftMetaData?.image && (
        <div className="relative w-full h-full group rounded-2xl border-4 border-white border-opacity-30 transition-all duration-500 hover:border-opacity-100 hover:shadow-lg">
          ...
          <div className="w-[40vw] md:w-64 flex flex-col text-white items-start">
            ...
            <p
              className={`md:text-base sm:text-sm text-xs font-medium mt-2 md:ml-4 ml-2 ${
                priceType === "auction" ? "text-blue-600" : ""
              }`}
            >
              {priceType === "auction"
                ? "Auction"
          ...
          <div
            className={`absolute bottom-0 left-0 w-full flex justify-center items-center bg-blue-600 text-white text-sm font-medium rounded-b-md opacity-0 ${
              priceType === "not_for_sale"
                ? ""
                : "group-hover:opacity-100 group-hover:translate-y-0 md:group-hover:h-[2.5rem] group-hover:h-[2rem] transition-all duration-300"
            } rounded-b-xl grid grid-cols-9 ${
              isProcessing === tokenId ? "opacity-100" : ""
            }`}
            ...
            {isProcessing === tokenId ? (
              ...
              <>
                ...
                {nftData.priceType === "fixed" && (
                  <>
                    <span className="col-span-6 mx-auto font-bold md:text-base sm:text-sm text-xs">
                      Buy Now
                    </span>
                    <div className="h-6 border-r-2 border-white mx-2"></div>
                    <img
                      className="col-span-2 md:h-6 md:w-6 h-4 w-4 mx-auto"
                      src="/buy.webp"
                    />
                  </>
                )}
